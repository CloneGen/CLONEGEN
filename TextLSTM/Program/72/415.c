int main () {
    ...
    for (i = 1; i < m - 1; i++) {
        if (s[i][0] >= s[i - 1][0] && s[i][0] >= s[i + 1][0] && s[i][0] >= s[i][1]) {
            printf ("%d 0\n", i);
        }
        for (k = 1; k < n - 1; k++) {
            if (s[i][k] >= s[i - 1][k] && s[i][k] >= s[i + 1][k] && s[i][k] >= s[i][k - 1] && s[i][k] >= s[i][k + 1]) {
                printf ("%d %d\n", i, k);
            }
        }
        if (s[i][n - 1] >= s[i - 1][n - 1] && s[i][n - 1] >= s[i + 1][n - 1] && s[i][n - 1] >= s[i][n - 2]) {
            printf ("%d %d\n", i, n - 1);
        }
    }
    ...
    for (a = 1; a < n - 1; a++) {
        if (s[m - 1][a] >= s[m - 1][a - 1] && s[m - 1][a] >= s[m - 1][a + 1] && s[m - 1][a] >= s[m - 2][a]) {
            printf ("%d %d\n", m - 1, a);
        }
    }
